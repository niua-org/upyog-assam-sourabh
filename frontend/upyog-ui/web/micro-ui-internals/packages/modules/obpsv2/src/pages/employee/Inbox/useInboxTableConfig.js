...
const useInboxTableConfig = ({ parentRoute, onPageSizeChange, formState, totalCount, table, dispatch, onSortingByData}) => {
    ...
    const tableColumnConfig = useMemo(() => {
        ...
        },
        {
            ...
            accessor: (row) => GetCell(row?.applicantName || row?.ownerName || "-"),
            disableSortBy: true,
            minWidth: 150,
        },
        {
            Header: t("FATHERS_NAME"),
            accessor: (row) => GetCell(row?.fatherName || row?.guardianName || "-"),
            disableSortBy: true,
            minWidth: 150,
        },
        {
            Header: t("MOBILE_NUMBER"),
            accessor: (row) => GetCell(row?.mobileNumber || row?.contactNumber || "-"),
            disableSortBy: true,
            minWidth: 120,
        },
        {
